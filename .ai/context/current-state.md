...
- Account and Bag omit themselves when WooCommerce or their configured pages are unavailable. No mini-cart, cart count, cart fragments, or product behavior exists.
- No external fonts are loaded; no Drop, release-state, private-access, archive, inventory, or M4 domain implementation has started.
- The core plugin remains unchanged from M1 at version `0.1.0`; the theme version also remains `0.1.0`.
- The next approved milestone is M4 — Drop Architecture + Product Metadata; it has not started.
...
- PHP syntax verification passes for all 14 first-party runtime PHP files.
...
M1/M2/M3 structural tests, JavaScript syntax, PHP bootstrap smoke, first-party PHP lint, and `node scripts/verify-foundation.mjs` pass with project-local PHP 8.3.33. Actual WordPress activation, browser rendering, and interactive assistive-technology behavior remain unverified. See `.ai/checks/m0-foundation.md`.
