...
test('Theme packaging generates single-root ZIP artifact with matching 0.13.0-rc.9 version', () => {
  const result = packageTheme('0.13.0-rc.9');
...
test('Plugin packaging generates single-root ZIP artifact with matching 0.13.0-rc.12 version', () => {
  const result = packagePlugin('0.13.0-rc.12');
  ...
  assert.equal(result.version, '0.13.0-rc.12');
  assert.ok(result.fileCount > 0, 'File count must be greater than 0');
  assert.ok(result.phpCount > 0, 'PHP file count must be greater than 0');
  assert.ok(result.sha256.length === 64, 'SHA-256 hash must be 64 hex characters');
});
...
test('Package verification confirms headers, constants, exclusions, and PHP syntax', () => {
  const themePath = resolve(root, 'dist', 'statement-collector-theme-0.13.0-rc.9.zip');
  const pluginPath = resolve(root, 'dist', 'statement-collector-core-0.13.0-rc.12.zip');

  const themeVerify = verifyPackage(themePath, '0.13.0-rc.9');
  ...
  assert.equal(themeVerify.headerVersion, '0.13.0-rc.9');
  assert.equal(themeVerify.constantVersion, '0.13.0-rc.9');

  const pluginVerify = verifyPackage(pluginPath, '0.13.0-rc.12');
  ...
  assert.equal(pluginVerify.headerVersion, '0.13.0-rc.12');
  assert.equal(pluginVerify.constantVersion, '0.13.0-rc.12');
});
...
test('packageAll orchestration generates verified artifacts and manifest without mutations', () => {
  ...
  assert.equal(manifest.plugin.header_version, '0.13.0-rc.12', 'plugin header_version must be 0.13.0-rc.12');
  assert.equal(manifest.plugin.runtime_version, '0.13.0-rc.12', 'plugin runtime_version must be 0.13.0-rc.12');
  assert.equal(manifest.theme.header_version, '0.13.0-rc.9', 'theme header_version must be 0.13.0-rc.9');
  assert.equal(manifest.theme.runtime_version, '0.13.0-rc.9', 'theme runtime_version must be 0.13.0-rc.9');
