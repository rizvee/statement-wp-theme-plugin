...
test('Theme packaging generates single-root ZIP artifact with matching 0.13.0-rc.13 version', () => {
  const result = packageTheme('0.13.0-rc.13');
...
test('Package verification confirms headers, constants, exclusions, and PHP syntax', () => {
  const themePath = resolve(root, 'dist', 'statement-collector-theme-0.13.0-rc.13.zip');
  const pluginPath = resolve(root, 'dist', 'statement-collector-core-0.13.0-rc.13.zip');

  const themeVerify = verifyPackage(themePath, '0.13.0-rc.13');
  ...
  assert.equal(themeVerify.headerVersion, '0.13.0-rc.13');
  assert.equal(themeVerify.constantVersion, '0.13.0-rc.13');
...
test('packageAll orchestration generates verified artifacts and manifest without mutations', () => {
  ...
  assert.equal(manifest.theme.header_version, '0.13.0-rc.13', 'theme header_version must be 0.13.0-rc.13');
  assert.equal(manifest.theme.runtime_version, '0.13.0-rc.13', 'theme runtime_version must be 0.13.0-rc.13');
