...
test('Plugin packaging generates single-root ZIP artifact with matching 0.13.0-rc.4 version', () => {
  const result = packagePlugin('0.13.0-rc.4');
...
test('Package verification confirms headers, constants, exclusions, and PHP syntax', () => {
  ...
  const pluginPath = resolve(root, 'dist', 'statement-collector-core-0.13.0-rc.4.zip');
  ...
  const pluginVerify = verifyPackage(pluginPath, '0.13.0-rc.4');
  ...
  assert.equal(pluginVerify.headerVersion, '0.13.0-rc.4');
  assert.equal(pluginVerify.constantVersion, '0.13.0-rc.4');
});
...
test('Master packageAll script generates manifest with candidate_versions and deployment_authorized = false', () => {
  ...
  assert.equal(manifest.plugin.header_version, '0.13.0-rc.4', 'plugin header_version must be 0.13.0-rc.4');
  assert.equal(manifest.plugin.runtime_version, '0.13.0-rc.4', 'plugin runtime_version must be 0.13.0-rc.4');
