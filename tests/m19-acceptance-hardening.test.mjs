...
test('M19 Acceptance Hardening & Production Convergence Suite', async (t) => {
  await t.test('1. Version Synchronization across Core, Theme, Demo, and Child Theme', () => {
    ...
    assert.match(themeStyle, /Version:\s*0\.13\.0-rc\.13/, 'Theme style.css must be 0.13.0-rc.13');

    const themeFuncs = readFileSync(join(root, 'wp-content/themes/statement-collector-theme/functions.php'), 'utf8');
    assert.match(themeFuncs, /STATEMENT_COLLECTOR_THEME_VERSION',\s*'0\.13\.0-rc\.13'/, 'Theme functions.php constant must be 0.13.0-rc.13');
    ...
    assert.match(demoEntry, /Version:\s*0\.2\.5/, 'Client demo header must be 0.2.5');
    assert.match(demoEntry, /STATEMENT_CLIENT_DEMO_VERSION',\s*'0\.2\.5'/, 'Client demo constant must be 0.2.5');
