...
test("M17: Theme modular structure and extension hooks", () => {
	...
	assert.ok(themeStyle.includes("Version: 0.13.0-rc.13"), "Theme style.css is version 0.13.0-rc.13");

	const functionsPhp = fs.readFileSync(path.join(themePath, "functions.php"), "utf8");
	assert.ok(functionsPhp.includes("STATEMENT_COLLECTOR_THEME_VERSION', '0.13.0-rc.13'"), "Theme constant is 0.13.0-rc.13");
});
