...
test('M14 Storefront Hardening: Theme Version & Constant Invariants (0.13.0-rc.20)', () => {
  ...
  assert.match(styleCss, /Version:\s*0\.13\.0-rc\.20/i, 'style.css must specify Version: 0.13.0-rc.20');
  assert.match(
    functionsPhp,
    /define\(\s*['"]STATEMENT_COLLECTOR_THEME_VERSION['"]\s*,\s*['"]0\.13\.0-rc\.20['"]\s*\);/,
    'functions.php must define STATEMENT_COLLECTOR_THEME_VERSION 0.13.0-rc.20'
  );
});
