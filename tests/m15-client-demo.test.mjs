...
test('M15: Theme 0.13.0-rc.19 metadata and file structure', () => {
...
test('M15: Package Theme 0.13.0-rc.19 and Client Demo 0.2.7', () => {
  const themePkg = packageTheme('0.13.0-rc.19');
  assert.equal(themePkg.version, '0.13.0-rc.19');
  assert.ok(themePkg.sizeBytes > 0);
  assert.ok(existsSync(themePkg.path));

  const themeVerify = verifyPackage(themePkg.path, '0.13.0-rc.19');
