...
test('M15: Package Theme 0.13.0-rc.12 and Client Demo 0.2.4', () => {
  const themePkg = packageTheme('0.13.0-rc.12');
  assert.equal(themePkg.version, '0.13.0-rc.12');
  assert.ok(themePkg.sizeBytes > 0);
  assert.ok(existsSync(themePkg.path));

  const themeVerify = verifyPackage(themePkg.path, '0.13.0-rc.12');
