...
test('M15: Theme 0.13.0-rc.6 metadata and file structure', () => {
  ...
  assert.ok(existsSync(join(root, 'wp-content/themes/statement-collector-theme/template-parts/home/lookbook.php')));
  assert.ok(existsSync(join(root, 'wp-content/themes/statement-collector-theme/template-parts/home/brand-object.php')));
...
test('M15: Client Demo plugin structure and security invariants', () => {
  ...
  assert.match(seederPhp, /monogram-jacquard-jacket/);
  assert.match(seederPhp, /panelled-hood-jacket/);

  const manifestPhp = readFileSync(join(root, 'tools/statement-client-demo/src/ManifestService.php'), 'utf8');
  assert.match(manifestPhp, /statement_client_demo_manifest_v1/);
...
test('M15: Package Theme 0.13.0-rc.6 and Client Demo 0.1.0', () => {
  const themePkg = packageTheme('0.13.0-rc.6');
  assert.equal(themePkg.version, '0.13.0-rc.6');
  assert.ok(themePkg.sizeBytes > 0);
  assert.ok(existsSync(themePkg.path));

  const themeVerify = verifyPackage(themePkg.path, '0.13.0-rc.6');
  assert.ok(themeVerify.ok, `Theme package verification failed: ${themeVerify.errors.join(', ')}`);

  const demoPkg = packageClientDemo('0.1.0');
  assert.equal(demoPkg.version, '0.1.0');
