...
test('M15: Theme 0.13.0-rc.20 metadata and file structure', () => {
  ...
  assert.match(navPhp, /function get_facebook_url/);
...
test('M15: Client Demo plugin structure and security invariants', () => {
  ...
  assert.match(seederPhp, /STMT-CD-D001-PHJ-XL/);
  ...
  assert.match(registryPhp, /statement-monogram-jacket-model-front\.webp/);
  assert.match(registryPhp, /statement-panelled-hood-jacket-model-front\.webp/);
});

test('M15: Package Theme 0.13.0-rc.20 and Client Demo 0.2.7', () => {
  const themePkg = packageTheme('0.13.0-rc.20');
  assert.equal(themePkg.version, '0.13.0-rc.20');
  assert.ok(themePkg.sizeBytes > 0);
  assert.ok(existsSync(themePkg.path));

  const themeVerify = verifyPackage(themePkg.path, '0.13.0-rc.20');
